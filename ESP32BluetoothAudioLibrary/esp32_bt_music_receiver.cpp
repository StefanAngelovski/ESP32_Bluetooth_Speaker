...
BluetoothA2DSink* actualBluetoothA2DSink;
i2s_port_t i2s_port; 
...
void BluetoothA2DSink::set_pin_config(i2s_pin_config_t pin_config){
  this->pin_config = pin_config;
}

void BluetoothA2DSink::set_i2s_port(i2s_port_t i2s_num) {
  i2s_port = i2s_num;
}

void BluetoothA2DSink::set_i2s_config(i2s_config_t i2s_config){
...
esp_a2d_audio_state_t BluetoothA2DSink::get_audio_state() {
  return audio_state;
}

esp_a2d_mct_t BluetoothA2DSink::get_audio_type() {
  return audio_type;
}


int BluetoothA2DSink::init_bluetooth()
{
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (!btStart()) {
...
bool BluetoothA2DSink::app_work_dispatch(app_callback_t p_cback, uint16_t event, void *p_params, int param_len)
{
...
void BluetoothA2DSink::app_work_dispatched(app_msg_t *msg)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
    if (msg->cb) {
...
bool BluetoothA2DSink::app_send_msg(app_msg_t *msg)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
    if (msg == NULL) {
        return false;
...
void BluetoothA2DSink::app_task_handler()
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
    app_msg_t msg;
    for (;;) {
...
void BluetoothA2DSink::app_task_start_up(void)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
...
void  BluetoothA2DSink::app_task_shut_down(void)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
    if (app_task_handle) {
...
void  BluetoothA2DSink::app_alloc_meta_buffer(esp_avrc_ct_cb_param_t *param)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
...
void  BluetoothA2DSink::app_rc_ct_callback(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
...
void  BluetoothA2DSink::av_hdl_a2d_evt(uint16_t event, void *p_param)
{
...
void  BluetoothA2DSink::av_new_track()
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
...
void  BluetoothA2DSink::av_notify_evt_handler(uint8_t event_id, uint32_t event_parameter)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
    switch (event_id) {
...
void  BluetoothA2DSink::av_hdl_avrc_evt(uint16_t event, void *p_param)
{
    ESP_LOGD(BT_AV_TAG, "%s evt %d", __func__, event);
...
void  BluetoothA2DSink::av_hdl_stack_evt(uint16_t event, void *p_param)
{
    switch (event) {
...
void  BluetoothA2DSink::app_a2d_callback(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
    ESP_LOGD(BT_AV_TAG, "%s", __func__);
    switch (event) {
    case ESP_A2D_CONNECTION_STATE_EVT:
...
void  BluetoothA2DSink::audio_data_callback(const uint8_t *data, uint32_t len) {
   ESP_LOGD(BT_AV_TAG, "%s", __func__);
...
void av_hdl_stack_evt_2(uint16_t event, void *p_param) {
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->av_hdl_stack_evt(event,p_param);
}
void av_hdl_a2d_evt_2(uint16_t event, void *p_param){
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->av_hdl_a2d_evt(event,p_param);  
}
void av_hdl_avrc_evt_2(uint16_t event, void *p_param){
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->av_hdl_avrc_evt(event,p_param);    
}
...
extern "C" void app_task_handler_2(void *arg) {
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->app_task_handler();
}

extern "C" void audio_data_callback_2(const uint8_t *data, uint32_t len) {
  //ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->audio_data_callback(data,len);
}

extern "C" void app_a2d_callback_2(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param){
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->app_a2d_callback(event, param);
}

extern "C" void app_rc_ct_callback_2(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param){
  ESP_LOGD(BT_AV_TAG, "%s", __func__);
  if (actualBluetoothA2DSink)
    actualBluetoothA2DSink->app_rc_ct_callback(event, param);
}
